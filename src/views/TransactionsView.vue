      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tr v-for="transaction in transactions" :key="transaction.id" class="hover:bg-gray-50">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ transaction.ID_paiement }}</td>
          ...
          <div class="flex min-h-full items-center justify-center p-4">
            ...
            >
              <DialogPanel class="w-full max-w-md transform overflow-hidden rounded-lg bg-white p-6 shadow-xl transition-all">
                ...
                <form @submit.prevent="editMode ? updateSelectedTransaction() : addTransaction()" class="space-y-4">
                  <div>
                  ...
                  <div>
                    ...
                    <input
                        v-model.number="txForm.paymentMethodId"
                        type="number"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 sm:text-sm"
                        required
                    />
...
function closeForm() {
  ...
  txForm.description = ''
  txForm.paymentMethodId = null
}

function editTransaction(transaction) {
  editMode.value = true
  txForm.id = transaction.id
  txForm.amount = transaction.amount
  txForm.date = new Date(transaction.date).toISOString().split('T')[0]
  txForm.description = transaction.description || ''
  txForm.paymentMethodId = transaction.ID_paiement
  showCreateForm.value = true
}

async function addTransaction() {
  await store.createTransaction({ ...txForm })
  if (!error) {
    closeForm()
  }
}

async function updateSelectedTransaction() {
  await store.updateTransaction(txForm.id, {
